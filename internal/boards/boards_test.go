...
func TestCreateBoardParams_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid kanban",
			params:  CreateParams{ProjectID: "proj-1", Name: "Main Board", Type: "kanban"},
			wantErr: false,
		},
		{
			name:    "valid scrum",
			params:  CreateParams{ProjectID: "proj-1", Name: "Sprint Board", Type: "scrum"},
			wantErr: false,
		},
		{
			name:    "missing project_id",
			params:  CreateParams{ProjectID: "", Name: "Main Board", Type: "kanban"},
			wantErr: true,
		},
		{
			name:    "missing name",
			params:  CreateParams{ProjectID: "proj-1", Name: "", Type: "kanban"},
			wantErr: true,
		},
		{
			name:    "invalid type",
			params:  CreateParams{ProjectID: "proj-1", Name: "Main Board", Type: "other"},
			wantErr: true,
		},
		{
			name:    "empty type",
			params:  CreateParams{ProjectID: "proj-1", Name: "Main Board", Type: ""},
			wantErr: true,
		},
	}
...
func TestCreateBoard_NilDB(t *testing.T) {
	_, err := Create(context.Background(), nil, CreateParams{ProjectID: "p", Name: "B", Type: "kanban"})
...
func TestListBoards_NilDB(t *testing.T) {
	_, err := List(context.Background(), nil, "proj-1")
	if err == nil || err.Error() != "db is required" {
